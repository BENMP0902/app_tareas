...
from methods import crear_cuenta, iniciar_sesion, encontrar_usuario_id, obtener_tareas_usuario, crear_tarea, actualizar_estado_tarea, eliminar_tarea
from flask_jwt_extended import decode_token, verify_jwt_in_request, get_jwt_identity, get_csrf_token, set_access_cookies
...
def cargar_rutas(app):
    ...
    def manipular_datos():
        ...
        respuesta = make_response(redirect(url_for('pantalla_usuario')))
        set_access_cookies(respuesta, respuesta_login['token'])
        respuesta.set_cookie('access_token_cookie', respuesta_login['token'], secure=True, httponly=True, max_age=600)
        return respuesta
    ...
    def obtener_tareas():
        try:
            ...
            # Obtiene las tareas del usuario con el ID obtenido
            tareas = obtener_tareas_usuario(id_usuario)
            print(tareas)

            return render_template('tasks.html', tareas=tareas)  # Muestra la página de tareas con las tareas obtenidas
        except Exception as e:
            print(e)
            return redirect(url_for('pagina'))

    @app.route('/actualizar_tarea/<int:tarea_id>', methods=['PUT'])
    def actualizar_tarea_ruta(tarea_id):
        try:
            verify_jwt_in_request()  # Verifica que el usuario esté autenticado
            data = request.get_json()  # Obtiene los datos enviados en formato JSON
            nuevo_estado = data.get('nuevo_estado')  # Obtiene el nuevo estado de la tarea

            if not nuevo_estado:
                return jsonify({'status': 'error', 'error': 'Falta el nuevo estado'}), 400

            # Llama a la función para actualizar el estado de la tarea
            respuesta = actualizar_estado_tarea(tarea_id, nuevo_estado)
    ...
    def eliminar_tarea_ruta(tarea_id):
        try:
            verify_jwt_in_request()  # Verifica que el usuario esté autenticado
            respuesta = eliminar_tarea(tarea_id)  # Llama a la función para eliminar la tarea
    ...
    def crear_tarea_ruta():
        try:
            verify_jwt_in_request()
            user_token = request.cookies.get('access_token_cookie')
            token_info = decode_token(user_token)
            id_usuario = token_info['user_id']

            data = request.get_json()
            nombre_tarea = data.get('name')
            descripcion_tarea = data.get('descripcion')

            if not nombre_tarea or not descripcion_tarea:
                return jsonify({'status': 'error', 'error': 'Faltan datos'}), 400

            respuesta = crear_tarea(id_usuario, nombre_tarea, descripcion_tarea)
