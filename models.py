...
# Modelo de Task (SQLAlchemy)
class Task(db_s.Model):
    __tablename__ = 'tasks'
    id = db_s.Column(db_s.Integer, primary_key=True)
    created_at = db_s.Column(db_s.DateTime, default=datetime.now)
    ...
    description = db_s.Column(db_s.Text, nullable=False)
    status = db_s.Column(db_s.String(50), default="pendiente")  # "pendiente", "en proceso", "completada"

    # Relación con el usuario
    usuario = db_s.relationship('Usuario', backref=db_s.backref('tasks', lazy=True))
